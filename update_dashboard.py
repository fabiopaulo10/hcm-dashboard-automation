"""
Fury Job — Dashboard HCM WH Operations
Atualiza dados do BigQuery, gera HTML e faz upload para Grid.
Agendar: 0 3 * * * (03:00 horário de Brasília)
"""
...
GRID_TOKEN   = os.environ["GRID_API_TOKEN"]
...
timecards_con_facility AS (
  SELECT *,
    COALESCE(timecard.EXPECTED_WORK_DAY.FACILITY_ID,
      (SELECT p.FACILITY_ID FROM UNNEST(timecard.PUNCHES) AS p LIMIT 1)) AS calculated_facility_id
  FROM `meli-bi-data.WHOWNER.BT_SHP_TYA_EMPLOYEE_TIMECARD` AS timecard
  WHERE DATE(APPLIED_FOR) >= DATE '2026-01-01'
),
presentismo AS (
  SELECT tc.calculated_facility_id AS facility_id, DATE(tc.APPLIED_FOR) AS fecha,
    asignaciones.TYPE AS tipo_contratacion,
    COUNT(DISTINCT CASE WHEN asignaciones.ID IS NOT NULL AND asignaciones.PROVIDER_ID IS NOT NULL THEN tc.EMPLOYEE_ID END) AS presentes_con_turno_con_provider,
    COUNT(DISTINCT CASE WHEN asignaciones.ID IS NOT NULL AND asignaciones.PROVIDER_ID IS NULL THEN tc.EMPLOYEE_ID END) AS presentes_con_turno_sin_provider,
    COUNT(DISTINCT CASE WHEN asignaciones.ID IS NULL THEN tc.EMPLOYEE_ID END) AS presentes_sin_turno
  FROM timecards_con_facility AS tc
  LEFT JOIN `meli-bi-data.WHOWNER.BT_SHP_MT_TYA_EMPLOYEE_SCHEDULE` AS asignaciones
    ON tc.ASSIGNMENT.ID = asignaciones.ID
  WHERE tc.ABSENCE.ID IS NULL AND COALESCE(tc.IS_DELETED, FALSE) IS FALSE
    ...
    AND tc.calculated_facility_id IN (SELECT FACILITY FROM facilities_activos)
  GROUP BY 1,2,3
),
dimensiones AS (
...
WHERE CASE WHEN d.facility_id='BRXSP8' THEN 'SSP26'
           WHEN d.facility_id='XPR1'   THEN 'SPR8'
           WHEN d.facility_id='XRJ1'   THEN 'SRJ1'
           ELSE d.facility_id END IN ('SSP9','SSP18','SSP29','SSP36')
...
def run_query():
    ...
    client = bigquery.Client(project=BQ_PROJECT)  # uses GOOGLE_APPLICATION_CREDENTIALS env var
...
def download_html():
    """Baixa o HTML atual do Grid."""
    headers = {"Authorization": f"Bearer {GRID_TOKEN}"}
    # Pegar download URL
    resp = requests.get(f"{GRID_API}/documents/{GRID_DOC_ID}", headers=headers)
    resp.raise_for_status()
    # Download direto
    dl = requests.get(f"{GRID_API}/documents/{GRID_DOC_ID}/download", headers=headers)
    dl.raise_for_status()
    info = dl.json()
    file_resp = requests.get(info["agent_download_url"])
    file_resp.raise_for_status()
    return file_resp.text


def update_html(html, rows):
    """Substitui EMBEDDED_DATA e DATA_END no HTML."""
    ...
    html = re.sub(
        r"const DATA_END = '.*?';",
        f"const DATA_END = '{data_end}';",
        html
    )
    return html, data_end


def upload_html(html_content):
    """Faz upload do HTML como nova versão no Grid."""
    import io
    headers = {"Authorization": f"Bearer {GRID_TOKEN}"}
    config = json.dumps({
    ...
    files = {
        ...
        "file": ("artifact_hcm.html", io.BytesIO(html_content.encode("utf-8")), "text/html"),
    }
    resp = requests.post(f"{GRID_API[:-3]}/engine/run", headers=headers, files=files)
    resp.raise_for_status()
    result = resp.json()
    if result.get("ok"):
        print(f"[Grid] Upload OK — versão {result.get('version')} | {result.get('view_url')}")
    else:
        print(f"[Grid] ERRO: {result}")
    return result
...
def main():
    ...
    print("2. Baixando HTML atual do Grid...")
    html = download_html()

    print("3. Atualizando dados no HTML...")
    html_updated, data_end = update_html(html, rows)
    print(f"   DATA_END atualizado para: {data_end}")

    print("4. Fazendo upload para Grid...")
    upload_html(html_updated)
